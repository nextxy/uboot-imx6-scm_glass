...
int ipu_probe(void)
{
	...
	// address in SRM Display General Control Register
	// IPU_MCU_T_DEFAULT The address space for accesses through the 
	// AHB-lite slave port is MB and it is split internally (with 32MB
//     resolution) according to bits [28:25] of the address. Using the following notation:
...
void ipu_dump_registers(void)
{
	...
	for(i=1;i<15;i++)
	{
		printf("IPU_INT_STAT_%d = \t0x%08X\n",
	       i,__raw_readl(IPU_INT_STAT(i)));
		printf("IPU_INT_CTL_%d = \t0x%08X\n",
	       i,__raw_readl(IPU_INT_CTRL(i)));
		
	}
		printf("IPU_STAT = \t0x%08X\n",
	       __raw_readl(IPU_STAT));
		printf("IPU_CHA_BUF0_RDY = \t0x%08X\n",
	       __raw_readl(IPU_CHA_BUF0_RDY(0)));
}
