...
// write IPU_CONF enable, init dp dc
...
// ipu_init_channel  ipu logic ch init
// ipu_dp_init ch = 9
// ipu_dc_init
// __raw_writel(ipu_conf, IPU_CONF); enable IPU_CONF
...
// set DI waveform, from var's value
// var's value must follow the vesa std
			...
			struct fb_videomode const *mode)
{
	...
	if (!g_dp_in_use) {
		mxcfbi->ipu_ch = MEM_BG_SYNC;  // ch = 9
		mxcfbi->blank = FB_BLANK_UNBLANK; // vesa no blank
	} else {
		mxcfbi->ipu_ch = MEM_DC_SYNC;  // ch = 7
	...
	// config dc di dp idmac 
	mxcfb_set_par(fbi);

	panel.winSizeX = mode->xres;  // pGD value
