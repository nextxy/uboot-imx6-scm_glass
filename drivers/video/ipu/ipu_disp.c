			    ...
			    uint32_t v_to_h_sync, ipu_di_signal_cfg_t sig)
{
	...
	printf("pixel_clk = %d\n", pixel_clk);
	printf("pixel_fmt = %d\n", pixel_fmt);
	printf("h_start_width = %d\n", h_start_width);
	printf("h_sync_width = %d\n", h_sync_width);
	printf("h_end_width = %d\n", h_end_width);
	printf("v_start_width = %d\n", v_start_width);
	printf("v_sync_width = %d\n", v_sync_width);
	printf("v_end_width = %d\n", v_end_width);
	printf("v_to_h_sync = %d\n", v_to_h_sync);
#endif
...
// The DI0_DW_GEN_<i> register holds pointers for the waveform generators.
	...
	if (sig.interlaced) {
		...
		/* Setup internal HSYNC waveform */
		// display timing set
		// DI output waveform setting
		printf("sig deinterlaced\n");
