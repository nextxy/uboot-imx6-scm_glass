...
static struct i2c_pads_info i2c_pad_info_0 = {
	.scl = {
...
// wyb
// epdc display controler did not use
#if defined(CONFIG_MX6DL) && defined(CONFIG_MXC_EPDC)
...
int mx6_rgmii_rework(struct phy_device *phydev)
{
         ...
         * additional tx data delay = -0.42ns, tx clk delay = 0.96ns, total 
= 1.38ns
...
.left_margin	= 220,	  h_start_width
.right_margin	= 40,	  h_end_width
.upper_margin	= 21,	 v_start_width
.lower_margin	= 7,	 v_end_width
.hsync_len		= 60,	 h_sync_width
.vsync_len		= 10,	 v_sync_width
.sync			= FB_SYNC_EXT,	=  4
.vmode			= FB_VMODE_NONINTERLACED	 =	0
...
// wyb default panel
		...
		.yres			= 1080,
		.pixclock		= 6734, // 6734 ps, 148500kHz
		.left_margin	= 148, // h start
		.right_margin	= 88,  // h end
		.upper_margin	= 36,  //v start
		.lower_margin	= 4,   //v end vesa
		.hsync_len		= 44,  //h sync vesa
		.vsync_len		= 5,   //v sync vesa
...
// wyb rgb lcos
	...
	.mode	= {
		...
		.xres           = 800,
		.yres           = 480,  //480
		.pixclock       = 17934,  //55.76MHz
		.left_margin    = 89, // 880 x 528
		.right_margin   = 164,
		.upper_margin   = 23,
		.lower_margin   = 10,
		.hsync_len      = 10,
...
int board_early_init_f(void)
{
	setup_iomux_uart();
#if defined(CONFIG_VIDEO_IPUV3)
printf("board_early init f set display\n");
	setup_display();
#endif
...
int board_init(void)
{
	...
	setup_spi();
#endif
//i2c3, 0x40 i2c probe max chip id
    setup_i2c(2, CONFIG_SYS_I2C_SPEED, 0x7f, &i2c_pad_info2);
//i2c2
	setup_i2c(1, CONFIG_SYS_I2C_SPEED, 0x7f, &i2c_pad_info1);
//i2c1, 0x40 i2c probe max chip id
        setup_i2c(0, CONFIG_SYS_I2C_SPEED, 0x7f, &i2c_pad_info_0);
...
int video_cvbs_init_board(void)
{
	int ret = 0;
	...
	ret = adv7280_capture(0);
	// ret = adv7280_capture(1);
